...
type Session interface {
	...
	// WrapKey wraps a key.
	WrapKey(m []*pkcs11.Mechanism, wrappingkey, key pkcs11.ObjectHandle) ([]byte, error)
	UnwrapKey(m []*pkcs11.Mechanism, unwrappingkey pkcs11.ObjectHandle, wrappedKey []byte, template []*pkcs11.Attribute) (Object, error)
	DestroyObject(key pkcs11.ObjectHandle)
...
func (s *SessionImpl) GenerateKey(mechanism []*pkcs11.Mechanism, template []*pkcs11.Attribute) (Object, error) {
	...
	object, err := s.Ctx.GenerateKey(s.Handle, mechanism, template)
	if err != nil {
		return Object{}, err
	}
...
// WrapKey -
func (s *SessionImpl) WrapKey(m []*pkcs11.Mechanism, wrappingkey, key pkcs11.ObjectHandle) ([]byte, error) {
	s.Lock()
	defer s.Unlock()

	wrappedKey, err := s.Ctx.WrapKey(s.Handle, m, wrappingkey, key)
	if err != nil {
		return nil, err
	}
...
// UnwrapKey -
func (s *SessionImpl) UnwrapKey(m []*pkcs11.Mechanism, unwrappingkey pkcs11.ObjectHandle, wrappedKey []byte, template []*pkcs11.Attribute) (Object, error) {
	s.Lock()
	defer s.Unlock()

	object, err := s.Ctx.UnwrapKey(s.Handle, m, unwrappingkey, wrappedKey, template)
	if err != nil {
		return Object{}, err
	}
...
// DestroyObject -
func (s *SessionImpl) DestroyObject(key pkcs11.ObjectHandle) {
	err := s.Ctx.DestroyObject(s.Handle, key)
	if err != nil {
		panic(err)
	}
}
